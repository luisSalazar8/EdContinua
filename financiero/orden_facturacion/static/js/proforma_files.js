...
$(document).on('change', ".proformaf", function (e) {
    e.preventDefault();
...
function cloneMore(selector, prefix) {
    ...
    newElement.attr("class","row"+(newrow) +" form-control form-row formset_row-proformafile_set")
    ...
    newElement.find("input[type=hidden] + div").each(function(){
        $(this).attr("id","div_id_proformafile_set-"+ (total)+"-file")
    });
    newElement.find("input[type=hidden] + div label").each(function(){
        $(this).attr("for","id_proformafile_set-"+ (total)+"-file")
    });
    newElement.find("div[class=form-check]").attr("id","div_id_proformafile_set-"+total+"-DELETE")
    total++;
        ...
        for (var i=1, formCount=forms.length; i<formCount; i++) {
            ...
            $(forms.get(i)).find(':input[type=hidden]').each(function() {
                if(lol==0){
                    ...
                    updateElementIndex(this, prefix, i,"-proforma");
                }
            });
        }
        var formset= $(".formset_row-proformafile_set");
        console.log(formset)
        for (var j=0, formsetCount=formset.length; j<formsetCount; j++) {
            console.log(formset[j]);
            var newrow=j+1
            console.log(newrow)
            $(formset.get(j)).attr("class","row"+newrow+"  form-control form-row formset_row-proformafile_set")
            var newElement=$(formset.get(j));

            newElement.find("input[type=hidden] + div").each(function(){
                $(this).attr("id","div_id_proformafile_set-"+ (j)+"-file")
            });
            newElement.find("input[type=hidden] + div label").each(function(){
                $(this).attr("for","id_proformafile_set-"+ (j)+"-file")
            });
            newElement.find("div[class=form-check]").attr("id","div_id_proformafile_set-"+j+"-DELETE")
        }
    }
    return false;
}
